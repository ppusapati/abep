"""Build hallthruster_bridge/propellants/dissociative_ionization_N2_N2+.dat (e + N2 -> N2+ + N + 3e, atomic dication
...
Above 1000 eV the last value is held (tail share printed). This script also prints the direct-vs-sequential N2+ source
comparison (sequential: ionization_N+_N2+.dat, Bell et al. 1983).
Usage: python scripts/build_n2_to_n2plus_table.py
"""
...
OUT = os.path.join(ROOT, "hallthruster_bridge", "propellants", "dissociative_ionization_N2_N2+.dat")
...
def main():
    ...
    print(" T_e   k_direct(N2->N2+)  k_seq(N+->N2+)  n_N+/n_N2 at which sequential = 5 % / 100 % of direct")
    for Te in (10, 15, 20, 25, 30):
        kd = maxwellian_rate(E, s, Te, TAIL); ks = audit.table_rate("ionization_N+_N2+.dat", Te)
    ...
    src = ("e + N2 -> N2+ + N + 3e. Cross section: Song et al., J. Phys. Chem. Ref. Data 52, 023104 (2023), Table 10 sigma(N++) "
           ...
           "appearance energy. Maxwellian-integrated by abep_sim/rate_tables.py (scripts/build_n2_to_n2plus_table.py). "
