"""Build hallthruster_bridge/propellants/ionization_N+_N2+.dat (e + N+ -> N2+ + 2e, sequential ionization).

Why it exists: the pre-registered audit promoted N2+ production (n2_completeness_audit_v1; N++ column of JPCRD 2023
Table 10 crosses F_ion = 1 % inside T_e <= 30 eV), so atomic N gets max_charge = 2. HallThruster.jl derives each species'
energy only through one-to-one reactions, so N2+ needs this N+ -> N2+ link to load at all; it is also the sequential
source route that must not be ignored once N2+ exists.
...
Usage: python scripts/build_n_plus_ionization_table.py
"""
import os, sys
import numpy as np
...
OUT = os.path.join(os.path.dirname(__file__), "..", "hallthruster_bridge", "propellants", "ionization_N+_N2+.dat")
HEADER_EV = 29.60125
...
def main():
    ...
    src = ("e + N+ -> N2+ + 2e. Cross section: Bell et al., J. Phys. Chem. Ref. Data 12, 891 (1983), Eq. (1), Table 5 N II "
           ...
           "(scripts/build_n_plus_ionization_table.py). Evidence level 4. Reaction set abep-n2n-0.5.")
