#!/bin/bash
# Pre-registered O4 escalation of n2_n_rot_off.toml (first-stage trigger FIRED, scored 2026-09-26; T_O4_ESCALATE via the transactional ledger).
S=/tmp/claude-0/-home-user-abep/275befef-ee58-5bbd-84f0-21c33eb50eb4/scratchpad
W=$S/wt_followon
cd $W
for M in escalation_n2_n_rot_off_di_lower escalation_n2_n_rot_off_nel_wang escalation_n2_n_rot_off_di_lower_nel_wang; do
  ...
  echo "$(date -u +%T) start $M ($MODE, $CHEM)" >> $S/followon/progress_escalation_rot_off.log
  for i in 0 1 2 3; do
  ...
  echo "$(date -u +%T) done $M structural_exit=$?" >> $S/followon/progress_escalation_rot_off.log
done
echo "$(date -u +%T) ALL DONE" >> $S/followon/progress_escalation_rot_off.log
