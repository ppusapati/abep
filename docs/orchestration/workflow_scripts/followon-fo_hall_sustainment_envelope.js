...
const ARGS_EMBEDDED = {"base": "fe1d79390ffa3a20183143fe84e9c5874da83004", "lanes": [{"key": "HSENV", "title": "fo_hall_sustainment_envelope", "allowed": ["docs/architecture_comparison/overlays/hall_sustainment/**", "tests/test_overlay_hall_sustainment.py"], "prompt": "Registered follow-on fo_hall_sustainment_envelope (trigger T_HALL_SUSTAIN_ENVELOPE; prerequisites verified). Map the published Hall-only sustainment/ignition evidence onto the COMMON FEED ENVELOPE to state where direct Hall operation (hall_only) on atmospheric propellant is evidence-supported, contradicted, or undetermined \u2014 conditional statements, not predictions, not a winner. Inputs (verified; read READ-ONLY from worktrees, reference by repository path; they will be merged with yours): lane_09_hall_sustainment at /home/user/abep/.claude/worktrees/wf_2bcadd98-5ff-3 (branch worktree-wf_2bcadd98-5ff-3, commit ac7970917f) (docs/evidence/hall_sustainment/*); lane_16_feed_envelope at /home/user/abep/.claude/worktrees/wf_3da51c0c-3b4-1 (branch worktree-wf_3da51c0c-3b4-1, commit ab27dcc449) (docs/architecture_comparison/feed_envelope/feed_envelope_v1.json, FEED_ENVELOPE.md). Deliverables in docs/architecture_comparison/overlays/hall_sustainment/: HALL_SUSTAINMENT_ENVELOPE.md + hall_sustainment_envelope_v1.json + deterministic build_hall_sustainment_envelope.py (--check reproduces): (1) for every feed-envelope case (altitude x atmosphere level, and any listed feed variants): the delivered anode flow / flow density / composition, and for each published sustainment evidence item whether its demonstrated operating region (flow, flow per channel area where geometry is reported, composition, voltage, B) covers, partially covers, or does not reach that case \u2014 with the explicit scaling assumption used to compare devices of different size (evidence-classed; e.g. flow per channel cross-section), or NOT_COMPARABLE with the missing quantity; (2) per case a status: SUSTAINMENT_SUPPORTED (evidence at comparable or harsher conditions), SUSTAINMENT_CONTRADICTED (evidence of extinction at comparable conditions), or UNDETERMINED (with blockers); feed-envelope TBD quantities propagate as UNDETERMINED, never invented; (3) Xe-assisted start vs air-only ignition, separately; (4) the conditions under which hall_only would satisfy the sustainment part of a milestone-A conditional baseline, and the single measurements that would resolve UNDETERMINED cases (feeds the minimum decisive experiment); (5) state that v1 simulation results are not sustainment evidence. No winner, no elimination (only via hard gates). Test tests/test_overlay_hall_sustainment.py: reproducible JSON, every case has status + evidence items or blockers, TBD propagation, no forbidden wording."}]}
