...
const ARGS_EMBEDDED = {"base": "c53b75a", "lanes": [{"key": "HALLREF", "title": "Lane 17 \u2014 Common Hall accelerator reference (interfaces)", "allowed": ["docs/architecture_comparison/hall_reference/**", "schemas/architecture_comparison/hall_reference_v1.schema.json", "tests/test_hall_reference.py"], "prompt": "Define ONE downstream Hall accelerator reference that is identical for hall_only, rf_hall and ecr_hall, so the RF/ECR comparison changes only the pre-ionization method. This is an INTERFACE definition for Vyovrinda's own thruster, not P5: never adopt P5 geometry or P5 B(z) as Vyovrinda's design. Deliverables: docs/architecture_comparison/hall_reference/HALL_ACCELERATOR_REFERENCE.md and hall_reference_v1.json (schema schemas/architecture_comparison/hall_reference_v1.schema.json) with: geometry interface (channel mean radius, width, length, anode position, exit plane, material; values TBD or owner-decided ranges, with sizing relations cited from open Hall-scaling literature as evidence, not as decisions); B(z) interface (parametrization compatible with the repository's B-field handling \u2014 read hallthruster_bridge/bfield/ and the HallThruster.jl case format used by the bridge, read-only \u2014 B_max, peak location, shape family; values TBD); discharge-voltage range (a PROPOSED range with rationale tied to the 12-25 mN / <1.5 kW envelope and sources); cathode interface (Xe-fed LaB6 per RFP context: electron current, flow, keeper; values TBD pending the cathode lanes); wall/life interface (fields that must match hallthruster_bridge/hall_map_schema_v1.json and abep_sim/hall_map.py REQUIRED_FIELDS, incl. wall_life_trustworthy semantics); the pre-ionizer injection interface (where RF/ECR ions/neutrals enter: anode/gas-distributor plane, charge-state and velocity distribution fields matching the interstage lane); and an invariance rule list (what may never differ between arms). State milestone A/B/C support. Test tests/test_hall_reference.py: schema validation; the wall/life field names exist in hall_map_schema_v1.json; every numeric value sourced, PROPOSED or TBD; the invariance list covers geometry, B(z), voltage set and cathode.", "repair": {"res": {"worktree_path": "/home/user/abep/.claude/worktrees/wf_3da51c0c-3b4-2", "branch": "worktree-wf_3da51c0c-3b4-2", "commit": "396d27cc39c220a0e60a72eddf6d26e05a14c321"}, "issues": [{"severity": "major", "file": "docs/architecture_comparison/hall_reference/HALL_ACCELERATOR_REFERENCE.md", "description": "The envelope tie in section 5 (lines 297-303), the proposed_range_V rationale in the JSON (line 946), and the 10 jet_power_* 'uncertainty' strings ('P_d = P_jet / efficiency, efficiency TBD') mix two incompatible efficiency definitions. P_jet here is the ideal-beam jet power T*v_b/2, where v_b = sqrt(2 e V_d / M). The text then lists published anodic efficiencies eta_a = T^2/(2 mdot_a P_d) (HT5k 8-18 %, BHT ~27 %, ECHT 13-23 %) and concludes 'so the ceiling is tight across the whole range'. That implies P_d ~ P_jet_ideal/eta_a. By the document's own ER-1/ER-2, P_d = T v_eff/(2 eta_a) with v_eff = gamma eta_m sqrt(eta_v) v_b. So P_jet_ideal/P_d = eta_a/(gamma eta_m sqrt(eta_v)), not eta_a, and the implied P_d is too high by 1/(gamma eta_m sqrt(eta_v)), about 3-5x for light propellants. The repository's own cited data contradict it. ECHT Run 2 (echt_table_checks_v1.json): 180 V, 22.76 mN at P_anode = 540 W. The ideal N2+ jet power at that thrust is about 0.40 kW, so P_jet_ideal/P_d ~ 0.74, while eta_a = 0.23; the implied inference gives ~1.7 kW instead of 0.54 kW. PPS1350-TSD gives 19-21 mN at ~1 kW. Fix: state explicitly that anodic efficiency must not be applied to the ideal-beam jet power. Either define the ratio used (eta* = P_jet_ideal/P_d = eta_a/(gamma eta_m sqrt(eta_v))) or tie the envelope to the measured light-propellant (T, P_d) pairs already cited (ECHT ~20-23 mN at 0.47-0.77 kW; PPS1350 19-21 mN at ~1 kW). Change the 'P_d = P_jet / efficiency' strings accordingly. The qualitative conclusion ('ceiling constrains the high-V_d, light-ion corner') can stay, but the stated quantitative chain is wrong physics."}, {"severity": "minor", "file": "docs/architecture_comparison/hall_reference/hall_reference_v1.json", "description": "ER-5 calls V_d >= (M/2e)(T/mdot_a)^2 a 'hard floor independent of the efficiencies'. That holds only for singly charged ions. ER-2 itself says gamma 'lumps divergence and multiply charged ions', and with doubly charged ions T/mdot_i can exceed sqrt(2 e V_b/M) by up to sqrt(2), so the floor can be undercut. Restrict the claim explicitly to a singly charged beam."}, {"severity": "minor", "file": "docs/architecture_comparison/hall_reference/HALL_ACCELERATOR_REFERENCE.md", "description": "SC-1 cites heavy_species_update.jl apply_left_boundary! as 'lines 303-420', but the function ends at line 429, and SC-2 cites line 422, which is inside it. Change the range to 303-429."}, {"severity": "minor", "file": "docs/architecture_comparison/hall_reference/hall_reference_v1.json", "description": "The SPT-50 Table 9-8 values (5 cm, 350 W, 20 mN) are classed 'measured'. Goebel & Katz Table 9-8 is a nominal performance summary with no measurement basis stated, so 'reported/inferred' would be more accurate."}, {"severity": "minor", "file": "docs/architecture_comparison/hall_reference/__pycache__", "description": "An untracked __pycache__ directory exists in the worktree under the deliverable folder (not committed). Make sure it is not added."}, {"severity": "minor", "file": "docs/architecture_comparison/hall_reference/hall_reference_v1.json", "description": "geometry_interface.fields.magnetically_shielded maps to the solver's Thruster.shielded, but bridge_lib.jl run_case builds het.Thruster(name, geometry, magnetic_field) without a shielded keyword. The bridge also always runs the default WallSheath(BNSiO2, 1.0). The reference states the wall-material default but does not mark either field as 'solver-supported, not wired in the bridge', the way it marks the inlet-state fields (SUPPORTED_NOT_WIRED)."}, {"severity": "minor", "file": "docs/architecture_comparison/hall_reference/hall_reference_v1.json", "description": "PROPOSED evaluation set [180, 200, 250, 300, 305] V puts two nodes 5 V apart. This follows from the rule 'both range ends plus the multiples of 50 V strictly inside', and it is disclosed as owner question Q1. It is still a questionable design-of-experiment choice for a map axis."}, {"severity": "minor", "file": "docs/architecture_comparison/hall_reference/hall_reference_v1.json", "description": "Every literature_operating_points[].discharge_voltage_V carries evidence_class 'measured', including the context-only 'SPT family' 200\u2013500 V range. That is a reported tested range, not a measured operating point. It does not affect the VR-1 span, because only the span_voltages_V of the span-defining points are used."}, {"severity": "minor", "file": "docs/architecture_comparison/hall_reference/HALL_ACCELERATOR_REFERENCE.md", "description": "VR-1 was written after the fact, over two repairs, and its criterion (c) was tightened after an adversarial review. The document discloses this openly (provenance_of_rule, Q1, and the alternative spans 180\u2013350 V and '<100'\u2013350 V are kept on record). The proposed 180\u2013305 V range therefore rests on an owner-pending rule, not a pre-declared one."}]}}]}
