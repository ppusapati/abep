...
const ARGS_EMBEDDED = {"base": "a170c1faa4c0b948efae96b3c862ee376b957a90", "lanes": [{"key": "RFOVL", "title": "fo_rf_breakeven_overlay", "allowed": ["docs/architecture_comparison/overlays/rf/**", "tests/test_overlay_rf.py"], "prompt": "Registered follow-on fo_rf_breakeven_overlay (trigger T_RF_OVERLAY; prerequisites verified). Place the RF source evidence on the architecture break-even surfaces, INCLUDING the interstage transport efficiency, to produce CONDITIONAL INEQUALITIES, not predictions or a winner. Inputs (verified; read READ-ONLY from their worktrees, reference by repository path; they will be merged with yours): lane_07_rf_evidence at /home/user/abep/.claude/worktrees/wf_2bcadd98-5ff-1 (branch worktree-wf_2bcadd98-5ff-1, commit 336d548042) (docs/evidence/rf_source/); lane_18_interstage at /home/user/abep/.claude/worktrees/wf_15f0f2f8-8d4-2 (branch worktree-wf_15f0f2f8-8d4-2, commit 870514285c) (abep_sim/interstage.py, docs/architecture_comparison/interstage/); lane_28_break_even at /home/user/abep/.claude/worktrees/wf_15f0f2f8-8d4-1 (branch worktree-wf_15f0f2f8-8d4-1, commit 2ad4c463d7) (abep_sim/breakeven.py incl. place_evidence, achievable_delivered_share, overhead_from_boundary_ledgers; docs/architecture_comparison/breakeven/); lane_20_ppu_magnet at /home/user/abep/.claude/worktrees/wf_15f0f2f8-8d4-3 (branch worktree-wf_15f0f2f8-8d4-3, commit f7c226848d) (electrical_closure_data_v1.json) for bus-referencing chains. You may import those modules by adding their worktree to sys.path in your SCRIPT ONLY at run time (never copy them). Deliverables in docs/architecture_comparison/overlays/rf/: RF_BREAKEVEN_OVERLAY.md + overlay_rf_v1.json + a deterministic script build_overlay_rf.py (--check reproduces the JSON): (1) the break-even condition for rf_hall stated as the owner asked: 'RF+Hall becomes preferable to direct Hall ONLY IF it delivers at least X (utilization gain / delivered ion share) at no more than Y bus W per delivered A and at least Z interstage transport efficiency', with X, Y, Z given as functions/tables over the PROPOSED analysis ranges and for the three response cases (add_only, cost_offset, optimistic_bound), on the common boundary bus_power_boundary_v1 (P_bus = all DC-bus power; never absorbed RF / source-only / discharge-only power); (2) every evidence entry that can be placed: its reported value and power reference (absorbed/forward/bus), the conversion chain to bus W per delivered A with sourced efficiencies (or the conversion that is missing), the interstage eta_t range from lane 18 with its basis, and its placement: CLEARLY_ABOVE_BREAKEVEN (cannot pay under any declared case and eta_t in range), CLEARLY_BELOW (can pay under at least the stated case), STRADDLES, or NOT_PLACEABLE (with the missing quantity); (3) the region of (eta_t, source ion cost) where rf_hall could pay, and whether ANY published evidence lies in it; (4) what single measurements would move each NOT_PLACEABLE / STRADDLES entry to a definite placement (feeds the minimum decisive experiment); (5) milestone-A statement: what can be concluded conditionally now, and the explicit conditions. No winner, no elimination (elimination is only via hard gates), no comparison with the other pre-ionizer. Test tests/test_overlay_rf.py: JSON reproducible, every placement has its conversion chain or missing-quantity note, boundary name bus_power_boundary_v1, no forbidden wording."}, {"key": "ECROVL", "title": "fo_ecr_breakeven_overlay", "allowed": ["docs/architecture_comparison/overlays/ecr/**", "tests/test_overlay_ecr.py"], "prompt": "Registered follow-on fo_ecr_breakeven_overlay (trigger T_ECR_OVERLAY; prerequisites verified). Place the ECR source evidence on the architecture break-even surfaces, INCLUDING the interstage transport efficiency, to produce CONDITIONAL INEQUALITIES, not predictions or a winner. Inputs (verified; read READ-ONLY from their worktrees, reference by repository path; they will be merged with yours): lane_08_ecr_evidence at /home/user/abep/.claude/worktrees/wf_2bcadd98-5ff-2 (branch worktree-wf_2bcadd98-5ff-2, commit a85fd59261) (docs/evidence/ecr_source/); lane_18_interstage at /home/user/abep/.claude/worktrees/wf_15f0f2f8-8d4-2 (branch worktree-wf_15f0f2f8-8d4-2, commit 870514285c) (abep_sim/interstage.py, docs/architecture_comparison/interstage/); lane_28_break_even at /home/user/abep/.claude/worktrees/wf_15f0f2f8-8d4-1 (branch worktree-wf_15f0f2f8-8d4-1, commit 2ad4c463d7) (abep_sim/breakeven.py incl. place_evidence, achievable_delivered_share, overhead_from_boundary_ledgers; docs/architecture_comparison/breakeven/); lane_20_ppu_magnet at /home/user/abep/.claude/worktrees/wf_15f0f2f8-8d4-3 (branch worktree-wf_15f0f2f8-8d4-3, commit f7c226848d) (electrical_closure_data_v1.json) for bus-referencing chains. You may import those modules by adding their worktree to sys.path in your SCRIPT ONLY at run time (never copy them). Deliverables in docs/architecture_comparison/overlays/ecr/: ECR_BREAKEVEN_OVERLAY.md + overlay_ecr_v1.json + a deterministic script build_overlay_ecr.py (--check reproduces the JSON): (1) the break-even condition for ecr_hall stated as the owner asked: 'ECR+Hall becomes preferable to direct Hall ONLY IF it delivers at least X (utilization gain / delivered ion share) at no more than Y bus W per delivered A and at least Z interstage transport efficiency', with X, Y, Z given as functions/tables over the PROPOSED analysis ranges and for the three response cases (add_only, cost_offset, optimistic_bound), on the common boundary bus_power_boundary_v1 (P_bus = all DC-bus power; never absorbed RF / source-only / discharge-only power); (2) every evidence entry that can be placed: its reported value and power reference (absorbed/forward/bus), the conversion chain to bus W per delivered A with sourced efficiencies (or the conversion that is missing), the interstage eta_t range from lane 18 with its basis, and its placement: CLEARLY_ABOVE_BREAKEVEN (cannot pay under any declared case and eta_t in range), CLEARLY_BELOW (can pay under at least the stated case), STRADDLES, or NOT_PLACEABLE (with the missing quantity); (3) the region of (eta_t, source ion cost) where ecr_hall could pay, and whether ANY published evidence lies in it; (4) what single measurements would move each NOT_PLACEABLE / STRADDLES entry to a definite placement (feeds the minimum decisive experiment); (5) milestone-A statement: what can be concluded conditionally now, and the explicit conditions. No winner, no elimination (elimination is only via hard gates), no comparison with the other pre-ionizer. Test tests/test_overlay_ecr.py: JSON reproducible, every placement has its conversion chain or missing-quantity note, boundary name bus_power_boundary_v1, no forbidden wording."}]}
