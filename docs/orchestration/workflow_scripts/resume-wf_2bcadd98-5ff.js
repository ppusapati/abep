...
const ARGS_EMBEDDED = {"base": "daa0e759e26416847f200a4781194f266be27c5c", "lanes": [{"key": "RF", "title": "RF ionization / pre-ionization source evidence audit (N2, O, O2, air)", "allowed": ["docs/evidence/rf_source/**", "tests/test_rf_source_evidence.py"], "prompt": "Evidence audit (no model implementation, no architecture conclusion) of RF plasma sources (inductively coupled, helicon, capacitively coupled; include RF-based ABEP heritage such as the SITAEL/ESA RAM-EP air-breathing tests, the IRS/Stuttgart inductive plasma thruster IPT for ABEP, and helicon ABEP concepts \u2014 only what open sources document) operating on N2, O2, O and air, as a pre-ionizer or ionization stage ahead of a Hall discharge. For each source record: gas, pressure/neutral density regime and the pressure/coupling limits (e.g. E-H mode transition, minimum pressure for inductive mode), RF frequency, forward vs absorbed power and power-transfer (coupling) efficiency including matching-network losses, RF generator DC-to-RF efficiency, ion production cost (W per A of extracted ion current or eV per ion) and ionization fraction, magnetic-field requirements (helicon), electrode/antenna/dielectric/wall interactions with atomic oxygen (oxidation, sputtering, erosion, contamination), thermal loads on coils/couplers, mass of RF hardware where stated. Deliverables: docs/evidence/rf_source/RF_SOURCE_EVIDENCE.md, rf_evidence_matrix.json (schema: id, source {citation, doi_or_url, access: open|abstract_only}, evidence_class, quantity, value, units, uncertainty, conditions {gas, pressure_Pa or density, power_W, frequency_Hz, B_T}, applicability_to_abep {regime_match: yes|partial|no|TBD, notes}), and rf_evidence_matrix.schema.json; a 'what must be measured' list (quantities the literature does not settle for ABEP-relevant flow/density, with the reason); a 'regime-match' table stating which ABEP intake/compressor densities are still TBD (they come from the upstream ICD; do not invent). No statement that RF pre-ionization is or is not worth its power: that is decided later on a common bus-power boundary. Test tests/test_rf_source_evidence.py: schema validation of every entry, every numeric entry has a source and evidence class, no entry lacks access labelling."}, {"key": "ECR", "title": "ECR ionization / pre-ionization source evidence audit (N2, O, O2, air, Xe)", "allowed": ["docs/evidence/ecr_source/**", "tests/test_ecr_source_evidence.py"], "prompt": "Evidence audit (no model implementation, no architecture conclusion) of electron-cyclotron-resonance plasma sources relevant to ABEP pre-ionization/ionization: microwave coupling (e.g. 2.45 GHz with 87.5 mT resonance, other frequencies), overdense operation limits (cutoff density; any documented overdense modes), absorbed vs forward power, microwave source DC-to-RF efficiency (magnetron, solid-state), magnet requirements (permanent-magnet mass, temperature limits, electromagnet power), usable ion-current gain / ion production cost (W/A, eV per ion), gas: N2, O2, air, O, Xe; heritage such as JAXA mu-series ECR ion thrusters and ECR neutralizers, ONERA ECR thruster, and any ECR air-breathing work documented in open sources; material interactions with atomic oxygen; thermal loads. Same deliverable structure as an evidence matrix: docs/evidence/ecr_source/ECR_SOURCE_EVIDENCE.md, ecr_evidence_matrix.json, ecr_evidence_matrix.schema.json (fields: id, source {citation, doi_or_url, access: open|abstract_only}, evidence_class, quantity, value, units, uncertainty, conditions {gas, pressure_Pa or density, power_W, frequency_Hz, B_T}, applicability_to_abep {regime_match: yes|partial|no|TBD, notes}); a 'what must be measured' list; a regime-match table with ABEP densities TBD from the upstream ICD. Physical constants you compute (e.g. cutoff density at a frequency) must show the formula and be labelled model-derived. No statement that ECR beats RF or vice versa. Test tests/test_ecr_source_evidence.py: schema validation, every numeric entry sourced and classified, formula-derived constants recomputed in the test."}, {"key": "HSUS", "title": "Hall-only sustainment / ignition evidence on N2, O2, air", "allowed": ["docs/evidence/hall_sustainment/**", "tests/test_hall_sustainment_evidence.py"], "prompt": "Evidence audit of published Hall-thruster operation on N2, O2, air and mixtures, to determine what the evidence says about whether a Hall discharge can ignite and sustain on atmospheric propellant WITHOUT a pre-ionizer, and under which conditions: ignition (with/without Xe start), minimum anode flow for sustained operation, discharge-voltage and magnetic-field ranges, channel length / geometry effects, oscillation modes and extinction, cathode flow and gas, anode/wall erosion observations, thrust and current where reported. Include the repository's own audited P5-N2 evidence (hallthruster_bridge/identification/brabston_p5_n2_measurement_audit_v1.json and findings; cite the files and their original sources) and the ECHT evidence status (hallthruster_bridge/identification/echt_n2/, numbers only; the thesis is CC BY-NC-ND and must not be redistributed), plus open published work such as SITAEL HT100/HT5k tests on N2/O2 (Andreussi et al., Cifali et al.), Busek air-breathing Hall work, two-stage Hall concepts for air, and others you can actually access. Deliverables: docs/evidence/hall_sustainment/HALL_SUSTAINMENT_EVIDENCE.md, hall_sustainment_matrix.json + schema (per entry: source/access, evidence_class, thruster, gas/mixture, flow, voltage, B, geometry, outcome {sustained|extinguished|unstable|not_reported}, quantities with units/uncertainty), and a per-evidence-item statement of what it implies for 'is pre-ionization required', with explicit uncertainty and applicability limits (flow/density regime vs ABEP is TBD from the upstream ICD). Do not use v1 simulation results as evidence of physical sustainment (they are model results; v1 is INCONCLUSIVE). No overall architecture conclusion. Test tests/test_hall_sustainment_evidence.py: schema validation, sourcing/evidence classes, repository-derived values match the repository files."}]}
