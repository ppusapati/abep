...
const ARGS_EMBEDDED = {"base": "c53b75a", "lanes": [{"key": "GATES", "title": "Lane 24 \u2014 Architecture hard-gate matrix and evaluator", "allowed": ["abep_sim/hard_gates.py", "schemas/architecture_comparison/hard_gates_v1.schema.json", "docs/architecture_comparison/hard_gates/**", "tests/test_hard_gates.py"], "prompt": "PRIORITY. Hard pass/fail conditions that let an architecture be ELIMINATED early without subjective scoring. docs/architecture_comparison/hard_gates/hard_gate_matrix_v1.json (+ schema schemas/architecture_comparison/hard_gates_v1.schema.json, + HARD_GATES.md): one gate per RFP requirement \u2014 thrust 12-25 mN over the 180-230 km envelope, bus power < 1.5 kW at the common DC boundary (bus_power_boundary_v1), mass MEV < 40 kg, firing life > 15,000 h, mission 26,000 h, restart/ignition and sustainment on atmospheric propellant, air + Xe operation \u2014 each with: metric definition, threshold and its RFP source (cite the RFP text/section as recorded in the repository; if the repository has only the CLAUDE.md summary, cite that and mark 'verify against RFP document'), the evidence required to declare FAIL (which evidence classes suffice, e.g. a hard physical bound or measured data; a model result from an unadmitted closure never suffices), the evidence required to declare PASS, and at which decision milestone (A conditional / B physics-backed / C PDR) each verdict can be issued. New pure module abep_sim/hard_gates.py: evaluate(architecture, evidence) -> per-gate verdict PASS / FAIL / UNDETERMINED with the evidence items used; missing or insufficient evidence => UNDETERMINED (never PASS by default); an architecture is ELIMINATED only when some gate is FAIL with sufficient evidence; also returns the conditions under which a conditional (milestone A) selection would hold. Include the owner's PROPOSED additional gates only if clearly marked PROPOSED (e.g. thermal feasibility, cathode life). Architecture ids exactly hall_only, rf_hall, ecr_hall. No winner. Test tests/test_hard_gates.py: UNDETERMINED by default, FAIL only with sufficient evidence class, elimination logic, milestone tagging, schema validation."}, {"key": "FTREE", "title": "Lane 26 \u2014 Architecture-specific failure trees", "allowed": ["docs/architecture_comparison/failure_tree/**", "schemas/architecture_comparison/failure_tree_v1.schema.json", "tests/test_failure_tree.py"], "prompt": "For each architecture (hall_only, rf_hall, ecr_hall) build a failure tree of the ways it could fail the programme: ignition failure, sustainment/extinction, utilization failure, interstage loss (rf_hall/ecr_hall), cathode limit (current, flow, poisoning, life), thermal limit (magnets, RF coil/coupler, ECR magnets, walls), erosion/wall-life limit, power limit (bus), mass limit, control/startup (Xe -> mixed -> atmosphere) failure, and any architecture-specific mode (e.g. RF E-H mode transition, ECR cutoff/overdense limit, magnetic-field interaction between ECR magnets and the Hall circuit). docs/architecture_comparison/failure_tree/failure_trees_v1.json (+ schema, + FAILURE_TREES.md): each node: id, architecture(s), mechanism, physical cause with open-literature evidence (source, evidence class), current evidence status (supported / contradicted / unknown), the quantity and threshold that decides it (linked to the hard-gate matrix ids where applicable: gates on thrust, bus power, mass, firing life, mission life, restart/sustainment, air+Xe), the cheapest evidence that would resolve it (literature, analysis, measurement) and which lane/deliverable would produce it (e.g. interstage model, break-even surfaces, electrical closure, cathode integration, minimum decisive experiment). Produce a ranked 'what evidence to acquire next' list per architecture by decisiveness (how many nodes/gates a single measurement resolves), with the ranking rule stated explicitly and no subjective weighting beyond that rule. No winner. Test tests/test_failure_tree.py: schema validation, every node has evidence status and a resolving action, gate links reference valid gate names, every architecture covers every generic failure class."}, {"key": "DOSSIER", "title": "Lane 27 \u2014 Continuously updated architecture decision-dossier generator", "deps": ["GATES", "FTREE"], "allowed": ["scripts/architecture/build_decision_dossier.py", "docs/architecture_comparison/dossier/**", "tests/test_decision_dossier.py"], "prompt": "Build a generator that assembles, from repository files only, the current architecture-decision dossier for hall_only, rf_hall and ecr_hall, so the decision is continuously updated instead of waiting for a final report. scripts/architecture/build_decision_dossier.py reads (when present; each missing input is listed as UNRESOLVED, never fabricated): the hard-gate matrix + evaluator (prerequisite lane), the failure trees (prerequisite lane), the evidence matrices (docs/evidence/rf_source, ecr_source, hall_sustainment, cathode, wall_life), the feed envelope, Hall accelerator reference, comparison grid, break-even surfaces, interstage model docs, electrical closure data, bus-power boundary, mass BOM (docs/architecture_comparison/mass_bom), thermal/life framework, scaling audit, minimum decisive experiment, cathode integration, the experiment protocol, gate status from CLAUDE.md, the P5-N2 v1 decision (hallthruster_bridge/validation/p5_n2_campaign_v1_vacuum_scores_decision.json) and VALIDATION_RELEASE_v1.json, and the forensics under docs/forensics/p5_n2_v1/. It writes docs/architecture_comparison/dossier/DECISION_DOSSIER.md and dossier.json: per architecture: hard-gate verdicts (PASS/FAIL/UNDETERMINED with evidence items), eliminated or not (only via the gate logic), which decision milestone is currently reachable (A conditional with the explicit conditions list / B / C) and what is missing for the next one, measured vs modelled vs assumed quantities with uncertainties, top unresolved failure-tree nodes and the next decisive evidence, and a provenance section (sha256 of every input file read, generation time, git commit). Deterministic apart from the timestamp. Must never present a winner unless the gate logic eliminates all but one AND the owner records a decision; say explicitly that the current dossier is a status, not a decision. Generate the current dossier from what exists in YOUR worktree + prerequisite outputs (copy nothing; read prerequisite files from their worktrees only for a preview run in a temp dir; the committed dossier must be built from your worktree's files and list the prerequisite-lane files as expected inputs pending merge). Test tests/test_decision_dossier.py: runs on the repository, missing inputs listed as UNRESOLVED, provenance hashes correct, no 'winner' output, deterministic."}]}
